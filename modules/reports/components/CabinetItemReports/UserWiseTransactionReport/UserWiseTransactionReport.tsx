...
import { ColumnOption, UserWiseTransactionDetail, UserWiseTransactionFilter } from "src/modules/reports/types/dto";
...
import { UserWiseTransactionReportFilter } from "./UserWiseTransactionReportFilter";
import { UserWiseTransactionReportGrid } from "./UserWiseTransactionReportGrid";
...
interface State {
    transactionsList: UserWiseTransactionDetail[];
...
const reportName = "TEXT_USER_WISE_TRANSACTION_REPORT";
const callPath = "/reports/overview";

export class UserWiseTransactionReport extends React.Component<Props, State>{

    columns: string[] = [
        "TEXT_USER_NAME", "TEXT_DESIGNATION", "TEXT_ROLE",
        "TEXT_TOTAL_TRANSACTION_COUNT",
        "TEXT_AVERAGE_KEY_TRANSACTION_HOLD_TIME",
        "TEXT_RETURN_OVERRIDE_COUNT",
        "TEXT_OVERDUE_RETURN_COUNT",
    ...
    performSearch(filters: UserWiseTransactionFilter, isShowMore: boolean = false) {
        ...
        reportService.getUserWiseTransactionList(selectedCustomerId, filters.site,
            filters.cabinetId, filters.userId, filters.jobTitle, filters.role, startDate, endDate, isShowMore ? continuationToken : undefined, limit)
            .then((response: any) => {
                this.setState({
                    ...this.state,
                    transactionsList: isShowMore ?
                        [...this.state.transactionsList || [], ...response.userTransactions.results] : response.userTransactions.results,
                    continuationToken: response.userTransactions.continuationToken,
                    isSearchInvoked: false,
    ...
    render() {
        ...
        return (
            <div className="report">
                <UserWiseTransactionReportFilter customerId={customerId} site={filter.site} cabinetId={filter.cabinetId} startDate={filter.startDate}
                    endDate={filter.endDate} periodDurationValue={filter.period} userId={filter.userId} jobTitle={filter.jobTitle} role={filter.role} periodString={periodString || ''}
                ...
                <Card>
                    <CardBody>
                        ...
                        <Row>
                            <Col>
                                <UserWiseTransactionReportGrid transactionList={transactionsList} selectedColumns={exportColumns} onScrollEnd={this.onScrollEnd} />
                            </Col>
                        </Row>
                        {
...
// WEBPACK FOOTER //
// ./src/modules/reports/components/CabinetItemReports/UserWiseTransactionReport/UserWiseTransactionReport.tsx